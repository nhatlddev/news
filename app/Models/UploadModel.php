...
class UploadModel extends BaseModel
{
    ...
    public function uploadBannerImage($tempPath, $width, $height)
    {
        $newPath = 'uploads/banner/' . $this->createUploadDirectory('banner') . 'image_' . $width . 'x_' . uniqid();
        $img = Image::make($tempPath)->orientate();
        $img->resize($width, null, function ($constraint) {
            $constraint->aspectRatio();
        });
    ...
    public function uploadCategoryImage($tempPath, $width, $height)
    {
        $newPath = 'uploads/category/' . $this->createUploadDirectory('category') . 'image_' . $width . 'x_' . uniqid();
        $img = Image::make($tempPath)->orientate();
        $img->resize($width, null, function ($constraint) {
            $constraint->aspectRatio();
        });
