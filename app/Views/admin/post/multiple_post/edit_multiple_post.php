<div class="row">
    <div class="col-sm-12">
        ...
        <input type="hidden" id="definition_id" name="definition_id" value="<?= $definition_id; ?>">
                ...
                <div class="tab-content">
                    <?php foreach ($activeLanguages as $index => $language): ?>
                        <div class="tab-pane <?= $index === 0 ? 'active' : ''; ?>" 
                            ...
                            <input type="hidden" name="posts[<?= $language->id; ?>][id]" value="<?= $postDataByLang[$language->id]['id']; ?>">

                            <div class="form-post">
                            <?= view('admin/post/multiple_post/edit_post_left', [
                                'language' => $language,
                                'post' => $postDataByLang[$language->id] ?? null
                            ]); ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                            ...
                            <select id="categories" name="posts-category_id" class="form-control category-selector" onchange="updateSubCategories(this.value);">
                                ...
                                <?php if (!empty($categories)):
                                    foreach ($categories as $item): ?>
                                        <option value="<?= $item->id; ?>" <?= $item->definition_id == $parentCategoryId ? 'selected' : ''; ?>><?= esc($item->name); ?></option>
                                    <?php endforeach;
                                endif; ?>
                            </select>
                        </div>
                            ...
                            <select id="subcategories" name="posts-subcategory_id" class="form-control subcategory-selector">
                                ...
                                <?php if (!empty($subCategories)): ?>
                                    <?php foreach ($subCategories as $subCategory): ?>
                                        <option value="<?= $subCategory->id; ?>" <?= $subCategory->definition_id == $subCategoryId ? 'selected' : ''; ?>>
...
<script>
    ...
    function submitFormsAsJSON(isDraft) {
        ...
        const data = {
            definition_id: document.getElementById('definition_id').value,
            posts: []
        };
        ...
        languageForms.forEach(form => {
            ...
            const post = {
                ...
                post_id: form.querySelector('input[name="posts[' + langId + '][id]"]').value,
                status: isDraft ? 0 : 1
            };
        ...
        fetch('<?= base_url("PostController/editPostPost2"); ?>', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?= csrf_hash(); ?>'
