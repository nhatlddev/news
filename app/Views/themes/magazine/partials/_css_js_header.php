<style>
    :root {
        --vr-font-primary:
            <?= getFontFamily($activeFonts, 'primary', true); ?>
        ;
        --vr-font-secondary:
            <?= getFontFamily($activeFonts, 'secondary', true); ?>
        ;
        --vr-font-tertiary:
            <?= getFontFamily($activeFonts, 'tertiary', true); ?>
        ;
        --vr-theme-color:
            <?= $activeTheme->theme_color; ?>
        ;
        --vr-block-color:
            <?= $activeTheme->block_color; ?>
        ;
        --vr-mega-menu-color:
            <?= $activeTheme->mega_menu_color; ?>
        ;
    }
...
<?php if ($activeTheme->theme == 'news'): ?>
    <style>
        ...
        <?php if ($rtl):
            echo ".section-featured .col-featured-left{padding-right:12px!important;padding-left:20px!important}.section-featured .col-featured-right{padding-left:20px!important;padding-right:0!important}.top-headlines{padding-left:12px!important;padding-right:0!important}@media (max-width: 991.98px){.section-featured .col-featured-left{padding-right:0!important;padding-left:0!important}.section-featured .col-featured-right{padding-left:0!important}.top-headlines{padding:0 15px!important}}";
        endif;
        if ($darkMode):
            echo "#nav-top .navbar-nav .nav-item .nav-link{color:#fdfdfd!important;font-weight:400}#nav-top .navbar-nav .nav-item .nav-link:hover,#nav-top .navbar-nav .nav-item .nav-link:active,#nav-top .navbar-nav .nav-item .nav-link:focus{color:#fdfdfd!important}.mega-menu .menu-left{background-image:none!important;background-color:#202020!important;padding-right:0!important}.top-headlines .item{border-color:#272727}.header-mobile,.mobile-search-form{background-color:#0f0f0f!important}.nav-mobile .nav-item .nav-link{color:#fdfdfd}.nav-mobile .btn-default{background-color:#1d1d1d!important;border-color:#1d1d1d!important;color:#fdfdfd!important}";
        endif; ?>
    </style><?php endif; ?>
<script>var VrConfig = { baseURL: '<?= base_url(); ?>', csrfTokenName: '<?= csrf_token() ?>', sysLangId: '<?= $activeLang->id; ?>', authCheck: <?= authCheck() ? 1 : 0; ?>, rtl: <?= $rtl ? 'true' : 'false'; ?>, isRecaptchaEnabled: '<?= isRecaptchaEnabled($generalSettings) ? 1 : 0; ?>', categorySliderIds: [], textOk: "<?= clrQuotes(trans("ok")); ?>", textCancel: "<?= clrQuotes(trans("cancel")); ?>", textCorrectAnswer: "<?= clrQuotes(trans("correct_answer")); ?>", textWrongAnswer: "<?= clrQuotes(trans("wrong_answer")); ?>" };</script>